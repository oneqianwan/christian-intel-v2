...
def isolated_runtime(tmp_path: Path):
    test_db_path = tmp_path / f"evidence_brief_no_llm_query_{uuid.uuid4().hex}.db"
...
def test_evidence_brief_simple_and_stream_queries_do_not_call_llm_or_real_network(isolated_runtime, monkeypatch):
    ...
    monkeypatch.setattr(
        brain_module.Brain,
        "_call_llm",
        lambda *_args, **_kwargs: pytest.fail("_call_llm must not be used by evidence brief query"),
    )
    monkeypatch.setattr(
        brain_module.Brain,
        "_call_llm_stream",
        lambda *_args, **_kwargs: pytest.fail("_call_llm_stream must not be used by evidence brief query"),
    )
    monkeypatch.setattr(
        socket,
        "create_connection",
    ...
    simple_result = brain.think("Give me an evidence brief for Harbor Church.", conversation_id="evidence-brief-no-llm-simple")
    stream_chunks = list(brain.think_stream("Give me an evidence brief for Harbor Church.", "evidence-brief-no-llm-stream"))
    stream_done = next(chunk for chunk in stream_chunks if isinstance(chunk, dict) and chunk.get("type") == "done")

    assert simple_result["partnership_evidence_brief"]["summary"]["brief_available"] is True
    assert stream_done["partnership_evidence_brief"]["summary"]["brief_available"] is True
