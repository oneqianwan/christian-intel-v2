...
def test_b3c3b_keeps_b3c3a_b3c2_b3c1_b3b_b2_b1_and_a3_readiness_contracts(startup_runtime):
    ...
    assert tenant_readiness["request_trace_tenant_scoped_ready"] == "partial"
    assert tenant_readiness["platform_diagnostics_super_admin_only_ready"] is True
...
def test_b3c3b_keeps_public_score_graph_and_contact_queries_stable(seeded_runtime, monkeypatch):
    brain_module = seeded_runtime["brain_module"]
    _block_llm_and_network(monkeypatch, brain_module)
    brain = brain_module.Brain()

    score_contract = brain.think("Victory Philippines 评分是多少？", conversation_id="b3c3b-reg-score")["response_contract"]
    graph_contract = brain.think("Victory Philippines 的关系图谱", conversation_id="b3c3b-reg-graph")["response_contract"]
    contact_contract = brain.think("Victory Philippines 怎么联系？", conversation_id="b3c3b-reg-contact")["response_contract"]
