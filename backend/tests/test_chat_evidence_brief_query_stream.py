...
def isolated_runtime(tmp_path: Path):
    test_db_path = tmp_path / f"chat_evidence_brief_stream_{uuid.uuid4().hex}.db"
...
def test_brain_stream_evidence_brief_query_returns_database_payload(isolated_runtime, monkeypatch):
    ...
    chunks = list(brain.think_stream("Give me an evidence brief for Harbor Church.", "evidence-brief-stream"))
    done_chunk = _done_chunk(chunks)
    payload = done_chunk["partnership_evidence_brief"]
    ...
    assert "priority" in payload["summary"]
    assert "confidence" in payload["summary"]
    assert "risk_level" in payload["summary"]
    assert "response_contract=partnership_evidence_brief" in done_chunk["full_content"]
    assert "llm_used=false" in done_chunk["full_content"]


def test_brain_stream_evidence_brief_query_handles_unavailable_without_fabrication(isolated_runtime, monkeypatch):
    runtime = isolated_runtime
    brain_module = runtime["brain_module"]
    _seed_org(runtime, org_id="org-source", name="Harbor Church")
    _block_llm_and_network(monkeypatch, brain_module)
    ...
    brain = brain_module.Brain()
    done_chunk = _done_chunk(list(brain.think_stream("给我 Harbor Church 的合作证据简报", "evidence-brief-stream-unavailable")))
    payload = done_chunk["partnership_evidence_brief"]

    assert payload["organization"]["name"] == "Harbor Church"
    assert payload["summary"]["brief_available"] is False
    assert "no_recommendation_candidates_found" in payload["warnings"]
    assert "status=brief_unavailable" in done_chunk["full_content"]


def test_brain_stream_evidence_brief_query_handles_org_and_target_not_found(isolated_runtime, monkeypatch):
    runtime = isolated_runtime
    brain_module = runtime["brain_module"]
    _seed_org(runtime, org_id="org-source", name="Harbor Church")
    _block_llm_and_network(monkeypatch, brain_module)
    ...
    org_not_found_done = _done_chunk(
        list(brain.think_stream("Give me an evidence brief for Missing Church.", "evidence-brief-stream-org-not-found"))
    )
    target_not_found_done = _done_chunk(
        list(brain.think_stream("给我 Harbor Church 联系 Missing Partner 的证据简报", "evidence-brief-stream-target-not-found"))
    )

    org_payload = org_not_found_done["partnership_evidence_brief"]
    target_payload = target_not_found_done["partnership_evidence_brief"]
