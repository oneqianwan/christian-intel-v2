...
def isolated_runtime(tmp_path: Path):
    test_db_path = tmp_path / f"action_plan_no_llm_{uuid.uuid4().hex}.db"
...
def test_action_plan_service_and_api_do_not_call_llm_or_real_network(isolated_runtime, monkeypatch):
    ...
    monkeypatch.setattr(
        llm_client,
        "call_llm",
        lambda *_args, **_kwargs: pytest.fail("call_llm must not be used by action plan service"),
    )
    monkeypatch.setattr(
        brain_module.Brain,
        "_call_llm",
        lambda *_args, **_kwargs: pytest.fail("_call_llm must not be used by action plan service"),
    )
    monkeypatch.setattr(
        brain_module.Brain,
        "_call_llm_stream",
        lambda *_args, **_kwargs: pytest.fail("_call_llm_stream must not be used by action plan service"),
    )
    monkeypatch.setattr(
        socket,
        "create_connection",
    ...
    assert api_payload["summary"]["plan_available"] is True
    assert api_payload["action_plan"]
