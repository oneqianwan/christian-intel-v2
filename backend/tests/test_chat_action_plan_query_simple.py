...
def isolated_runtime(tmp_path: Path):
    test_db_path = tmp_path / f"chat_action_plan_simple_{uuid.uuid4().hex}.db"
...
def test_brain_simple_action_plan_query_returns_database_payload(isolated_runtime, monkeypatch):
    ...
    result = brain.think("What are the next steps for Harbor Church?", conversation_id="action-plan-simple")

    payload = result["partnership_action_plan"]
    ...
    assert payload["summary"]["recommended_channel"] in {"email", "website", "phone", "social", "manual_review", "research_first"}
    ...
    assert "response_contract=partnership_action_plan" in result["answer"]
    assert "recommended_channel:" in result["answer"]
    assert "risk_level:" in result["answer"]
    assert "llm_used=false" in result["answer"]


def test_brain_simple_action_plan_query_handles_blocked_and_no_fabrication(isolated_runtime, monkeypatch):
    runtime = isolated_runtime
    brain_module = runtime["brain_module"]
    _seed_org(runtime, org_id="org-source", name="Harbor Church")
    _block_llm_and_network(monkeypatch, brain_module)
    ...
    brain = brain_module.Brain()
    result = brain.think("给我 Harbor Church 的合作行动计划", conversation_id="action-plan-simple-blocked")
    ...
    assert "当前数据库没有足够数据生成可执行行动计划" in result["answer"]
    assert "我不会编造联系人、关系路径或外联步骤" in result["answer"]


def test_brain_simple_action_plan_query_without_contact_does_not_emit_contact_step(isolated_runtime, monkeypatch):
    runtime = isolated_runtime
    brain_module = runtime["brain_module"]
    _seed_org(runtime, org_id="org-source", name="Harbor Church")
    _seed_org(
        ...
        phone_public=None,
        facebook_url=None,
        source_url="https://waypoint.example/source",
    )
    _seed_edge(runtime, edge_id="edge-source-waypoint", source_id="org-source", target_id="org-target")
    _block_llm_and_network(monkeypatch, brain_module)

    brain = brain_module.Brain()
    result = brain.think("What are the next steps for Harbor Church?", conversation_id="action-plan-simple-no-contact")
...
def test_brain_simple_action_plan_query_handles_org_and_target_not_found(isolated_runtime, monkeypatch):
    runtime = isolated_runtime
    brain_module = runtime["brain_module"]
    _seed_org(runtime, org_id="org-source", name="Harbor Church")
    _block_llm_and_network(monkeypatch, brain_module)
    ...
    brain = brain_module.Brain()
    org_not_found = brain.think("What are the next steps for Missing Church?", conversation_id="action-plan-simple-org-not-found")
    target_not_found = brain.think(
        "给我 Harbor Church 联系 Missing Partner 的行动计划",
        conversation_id="action-plan-simple-target-not-found",
    )

    org_payload = org_not_found["partnership_action_plan"]
    target_payload = target_not_found["partnership_action_plan"]
    ...
    assert target_payload["summary"]["blocked"] is True
