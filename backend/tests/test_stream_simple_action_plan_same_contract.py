...
def isolated_runtime(tmp_path: Path):
    test_db_path = tmp_path / f"action_plan_same_contract_{uuid.uuid4().hex}.db"
...
def test_simple_and_stream_action_plan_return_same_contract(isolated_runtime, monkeypatch):
    ...
    message = "What are the next steps for Harbor Church?"
    brain = brain_module.Brain()
    simple_result = brain.think(message, conversation_id="action-plan-same-simple")
    stream_chunks = list(brain.think_stream(message, "action-plan-same-stream"))
    stream_done = next(chunk for chunk in stream_chunks if isinstance(chunk, dict) and chunk.get("type") == "done")

    simple_payload = simple_result["partnership_action_plan"]
    stream_payload = stream_done["partnership_action_plan"]
    ...
    assert simple_payload["evidence"] == stream_payload["evidence"]
    assert simple_payload["warnings"] == stream_payload["warnings"]
    assert simple_payload["action_plan"]
    for step in simple_payload["action_plan"]:
