...
def isolated_runtime(tmp_path: Path):
    test_db_path = tmp_path / f"graph_same_contract_{uuid.uuid4().hex}.db"
...
def test_simple_and_stream_relationship_graph_return_same_contract(isolated_runtime, monkeypatch):
    ...
    message = "Show me the relationship graph of Victory Philippines"
    brain = brain_module.Brain()
    simple_result = brain.think(message, conversation_id="graph-same-simple")
    stream_chunks = list(brain.think_stream(message, "graph-same-stream"))
    stream_done = next(chunk for chunk in stream_chunks if isinstance(chunk, dict) and chunk.get("type") == "done")

    assert "relationship_graph" in simple_result
    assert "relationship_graph" in stream_done
    assert simple_result["answer"] == stream_done["full_content"]
    assert simple_result["relationship_graph"] == stream_done["relationship_graph"]
    for snippet in [
        "response_contract=relationship_graph",
        "Victory Philippines",
        "Life.Church",
        "relation_type: partner",
        "evidence: https://example.com/partner-proof",
        "confidence: 0.82",
        "is_verified: true",
        "llm_used=false",
    ]:
    ...
    assert "action plan" not in simple_result["answer"].lower()
