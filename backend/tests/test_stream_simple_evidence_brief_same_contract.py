...
def isolated_runtime(tmp_path: Path):
    test_db_path = tmp_path / f"evidence_brief_same_contract_{uuid.uuid4().hex}.db"
...
def test_simple_and_stream_evidence_brief_return_same_contract(isolated_runtime, monkeypatch):
    runtime = isolated_runtime
    brain_module = runtime["brain_module"]
    _seed_org(runtime, org_id="org-source", name="Harbor Church", contact_email="hello@harbor.org", source_url="https://harbor.org/source")
    _seed_org(runtime, org_id="org-target", name="Bridge Ministry", people_score=78, digital_score=74, intel_score=81, contact_email="connect@bridge.org", phone_public="+63 2 1234 5678", source_url="https://bridge.org/source")
    ...
    message = "Give me an evidence brief for Harbor Church."
    brain = brain_module.Brain()
    simple_result = brain.think(message, conversation_id="evidence-brief-same-simple")
    stream_chunks = list(brain.think_stream(message, "evidence-brief-same-stream"))
    stream_done = next(chunk for chunk in stream_chunks if isinstance(chunk, dict) and chunk.get("type") == "done")

    simple_payload = simple_result["partnership_evidence_brief"]
    stream_payload = stream_done["partnership_evidence_brief"]
    ...
    assert simple_payload["target_org"] == stream_payload["target_org"]
    assert simple_payload["summary"] == stream_payload["summary"]
    assert simple_payload["decision_rationale"] == stream_payload["decision_rationale"]
    assert simple_payload["risk_register"] == stream_payload["risk_register"]
    assert simple_payload["audit"] == stream_payload["audit"]
    assert simple_payload["warnings"] == stream_payload["warnings"]
    assert set(simple_payload["evidence_sections"].keys()) == set(stream_payload["evidence_sections"].keys())
    assert simple_payload["audit"]["no_llm"] is True
