...
def runtime(tmp_path: Path):
    test_db_path = tmp_path / f"observability_fallback_{uuid.uuid4().hex}.db"
...
def test_missing_org_context_reference_and_unsupported_have_safe_fallback_audit(runtime, monkeypatch):
...
def test_ambiguous_org_fallback_audit_does_not_fabricate_id(runtime, monkeypatch):
    brain_module = runtime["brain_module"]
    monkeypatch.setattr(
        brain_module.Brain,
