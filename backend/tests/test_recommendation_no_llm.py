...
def isolated_runtime(tmp_path: Path):
    test_db_path = tmp_path / f"recommendation_no_llm_{uuid.uuid4().hex}.db"
...
def _seed_org(runtime, *, org_id: str, name: str, email: str | None = None, website: str | None = "https://example.org/source"):
    ...
    try:
        org = database.OrganizationProfile(
            ...
            digital_score=58,
            intel_score=67,
            official_website=website.replace("/source", "") if website else None,
            contact_email=email,
            source_url=website,
...
def test_recommendation_service_and_api_do_not_call_llm_or_real_network(isolated_runtime, monkeypatch):
    ...
    _seed_org(runtime, org_id="org-source", name="Harbor Church", email="hello@harbor.org", website="https://harbor.org/source")
    _seed_org(runtime, org_id="org-target", name="Bridge Ministry", email="connect@bridge.org", website="https://bridge.org/source")
    _seed_edge(runtime, edge_id="edge-source-target", source_id="org-source", target_id="org-target")

    monkeypatch.setattr(
        llm_client,
        "call_llm",
        lambda *_args, **_kwargs: pytest.fail("call_llm must not be used by recommendation service"),
    )
    monkeypatch.setattr(
        brain_module.Brain,
        "_call_llm",
        lambda *_args, **_kwargs: pytest.fail("_call_llm must not be used by recommendation service"),
    )
    monkeypatch.setattr(
        brain_module.Brain,
        "_call_llm_stream",
        lambda *_args, **_kwargs: pytest.fail("_call_llm_stream must not be used by recommendation service"),
    )
    monkeypatch.setattr(
        socket,
        "create_connection",
    ...
    assert api_payload["found"] is True
    assert api_payload["recommendations"]
