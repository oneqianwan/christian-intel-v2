...
def seeded_runtime(tmp_path: Path):
    test_db_path = tmp_path / f"observability_consistency_{uuid.uuid4().hex}.db"
...
def test_simple_and_stream_observability_match_for_ready_routes(seeded_runtime, monkeypatch, message):
    ...
    simple = brain.think(message, conversation_id=f"simple-{uuid.uuid4().hex}")["response_contract"]["observability"]["audit_summary"]
    stream_done = next(
        item for item in brain.think_stream(message, f"stream-{uuid.uuid4().hex}") if isinstance(item, dict) and item.get("type") == "done"
    )
    stream = stream_done["response_contract"]["observability"]["audit_summary"]
    ...
    assert simple["data_found"] == stream["data_found"]
    assert simple["trace_id"] != stream["trace_id"]


def test_simple_and_stream_observability_match_for_safe_fallback(seeded_runtime):
    brain = seeded_runtime["brain_module"].Brain()
    simple = brain.think("推荐合作对象有哪些？", conversation_id=f"simple-fallback-{uuid.uuid4().hex}")["response_contract"]["observability"]["audit_summary"]
    stream_done = next(
        item
    ...
    assert simple["payload_type"] == stream["payload_type"]
    assert simple["selected_module"] == stream["selected_module"]
    assert simple["safe_fallback_used"] == stream["safe_fallback_used"] is True
