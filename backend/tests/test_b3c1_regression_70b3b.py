...
def test_b3c1_keeps_b3b_b2_b1_and_a3_readiness_contracts(startup_runtime):
    ...
    assert report["tenant_readiness"]["private_tenant_schema_ready"] in {"yes", "partial"}
...
def test_b3c1_keeps_public_score_and_graph_queries_stable(seeded_runtime, monkeypatch):
    brain_module = seeded_runtime["brain_module"]
    _block_llm_and_network(monkeypatch, brain_module)
    brain = brain_module.Brain()

    score_result = brain.think("Victory Philippines 评分是多少？", conversation_id="b3c1-reg-score")
    graph_result = brain.think("Victory Philippines 的关系图谱", conversation_id="b3c1-reg-graph")

    assert score_result["response_contract"]["payload_type"] == "score_snapshot"
    assert graph_result["response_contract"]["payload_type"] == "relationship_graph"
    assert score_result["response_contract"]["audit"]["no_llm"] is True
    assert graph_result["response_contract"]["audit"]["no_network"] is True
