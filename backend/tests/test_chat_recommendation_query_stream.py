...
def isolated_runtime(tmp_path: Path):
    test_db_path = tmp_path / f"chat_recommendation_stream_{uuid.uuid4().hex}.db"
...
def test_brain_stream_recommendation_query_returns_database_payload(isolated_runtime, monkeypatch):
    ...
    chunks = list(brain.think_stream("Who should Victory Philippines partner with?", "recommendation-stream"))
    done_chunk = _done_chunk(chunks)
    payload = done_chunk["partnership_recommendations"]
    ...
    assert "response_contract=partnership_recommendations" in done_chunk["full_content"]
    assert "Bridge Ministry" in done_chunk["full_content"]
    assert "llm_used=false" in done_chunk["full_content"]


def test_brain_stream_recommendation_query_handles_no_candidates_without_fabrication(isolated_runtime, monkeypatch):
    runtime = isolated_runtime
    brain_module = runtime["brain_module"]
    _seed_org(runtime, org_id="org-victory", name="Victory Philippines")
    _block_llm_and_network(monkeypatch, brain_module)
    ...
    brain = brain_module.Brain()
    chunks = list(brain.think_stream("推荐 Victory Philippines 适合合作的机构", "recommendation-stream-empty"))
    done_chunk = _done_chunk(chunks)
    payload = done_chunk["partnership_recommendations"]
    ...
    assert "status=no_candidates" in done_chunk["full_content"]
    assert "我不会编造推荐机构" in done_chunk["full_content"]


def test_brain_stream_recommendation_query_handles_org_not_found(isolated_runtime, monkeypatch):
    runtime = isolated_runtime
    brain_module = runtime["brain_module"]
    _block_llm_and_network(monkeypatch, brain_module)

    brain = brain_module.Brain()
    chunks = list(brain.think_stream("Recommend partner organizations for Unknown Church.", "recommendation-stream-not-found"))
    done_chunk = _done_chunk(chunks)
    payload = done_chunk["partnership_recommendations"]
    ...
    assert payload["recommendations"] == []
    assert "status=not_found" in done_chunk["full_content"]
    assert "I will not fabricate recommendation targets" in done_chunk["full_content"] or "我不会编造推荐对象" in done_chunk["full_content"]
