...
def _apply_migration(database, auth_models) -> None:
    with database.engine.begin() as conn:
        database.Base.metadata.create_all(
            bind=conn,
            tables=[auth_models.User.__table__, auth_models.AuthSession.__table__],
        )
...
def _verify_schema(database, auth_models) -> VerifyResult:
    ...
    if missing_users_cols or missing_sessions_cols:
        return VerifyResult(
            ok=False,
            details={
    ...
    forbidden_columns = [
        name
        for name in sorted(users_cols | sessions_cols)
    ...
    if missing_unique_users or missing_unique_sessions:
        return VerifyResult(
            ok=False,
            details={
